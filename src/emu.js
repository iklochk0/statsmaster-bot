import { execa } from "execa";

const ADB = process.env.ADB_BIN || "adb";
const SERIAL = process.env.ADB_SERIAL || "";

function adbArgs(args) {
  const a = [];
  if (SERIAL) a.push("-s", SERIAL);
  a.push(...args);
  return a;
}

export const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

export async function tap(x, y) {
  await execa(
    ADB,
    adbArgs(["shell", "input", "tap", String(x), String(y)]),
    { encoding: "buffer" }
  );
}

export async function swipe(x1, y1, x2, y2, durMs = 300) {
  await execa(
    ADB,
    adbArgs([
      "shell",
      "input",
      "swipe",
      String(x1),
      String(y1),
      String(x2),
      String(y2),
      String(durMs),
    ]),
    { encoding: "buffer" }
  );
}

export async function back() {
  await execa(ADB, adbArgs(["shell", "input", "keyevent", "4"]), {
    encoding: "buffer",
  });
}

export async function home() {
  await execa(ADB, adbArgs(["shell", "input", "keyevent", "3"]), {
    encoding: "buffer",
  });
}

export async function navigate(nav) {
  if (!nav) return;
  if (nav.type === "tap") return tap(nav.x, nav.y);
  if (nav.type === "swipe") {
    return swipe(nav.x1, nav.y1, nav.x2, nav.y2, nav.durMs || 300);
  }
  throw new Error(`Unknown nav type: ${JSON.stringify(nav)}`);
}

export async function getClipboardText() {
  try {
    const { stdout } = await execa(
      ADB,
      adbArgs(["shell", "cmd", "clipboard", "get"]),
      { encoding: "utf8" }
    );
    return (stdout || "").trim();
  } catch {}

  // Older Android images can expose clipboard data through service call.
  try {
    const { stdout } = await execa(
      ADB,
      adbArgs(["shell", "service", "call", "clipboard", "1"]),
      { encoding: "utf8" }
    );
    const s = (stdout || "").trim();
    if (/Parcel|Result|data/gi.test(s) && !/^\w[\s\S]{0,200}$/.test(s)) return "";
    return s;
  } catch {}
  return "";
}
