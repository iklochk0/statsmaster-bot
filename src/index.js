...
const USE_HOST_CLIPBOARD = process.env.USE_HOST_CLIPBOARD !== "false";
...
async function sleepLog(ms, reason = "") { logAction("sleep", { ms, reason }); await sleep(ms); }
...
let ZONE_MODE = null; // { code, mode }
if (POSITIONAL[0] === "zone" && POSITIONAL.length >= 3) {
  const code = String(POSITIONAL[1]);
  const m = String(POSITIONAL[2] || "").toLowerCase();
  if (m === "start" || m === "finish") {
    ZONE_MODE = { code, mode: m };
    console.log(`Zone mode: ${code} → ${m}`);
  } else {
    console.warn(`Zone args ignored (expected: "zone <code> start|finish")`);
  }
}

/* ===================== ADB helpers ===================== */
function adbArgs(args) { const a=[]; if (SERIAL) a.push("-s", SERIAL); a.push(...args); return a; }

async function adbTapExact(x, y, durMs = 120, meta = {}) {
  const xx = Math.round(x), yy = Math.round(y), dd = Math.max(60, Math.round(durMs));
  logAction("tap", { x: xx, y: yy, durMs: dd, ...meta });
  await execa(ADB, adbArgs(["shell","input","swipe", String(xx), String(yy), String(xx), String(yy), String(dd)]), { encoding: "buffer" });
}
async function adbSwipe(x1, y1, x2, y2, durMs = 300, meta = {}) {
  ...
  await execa(ADB, adbArgs(["shell","input","swipe", String(xx1), String(yy1), String(xx2), String(yy2), String(dd)]), { encoding: "buffer" });
}
...
async function fakeSwipeInDead() {
  for (let i = 0; i < FAKE_SWIPE_COUNT; i++) {
    ...
    await adbSwipe(x0, y0, x0, y0 - FAKE_SWIPE_DY, FAKE_SWIPE_DUR + randInt(-60, 60), { i });
...
async function navigateHuman(actionOrArray) {
  ...
  if (Array.isArray(actionOrArray)) { for (const a of actionOrArray) await doOne(a); }
  else { await doOne(actionOrArray); }
}
...
function levelRectForRow(i) {
  ...
  const safeTop = SAFE.top, safeBottom = SAFE.top + SAFE.height;
...
async function isProfileScreen() {
  ...
  const killsRaw = rois.kills ? (await ocrBuffer(rois.kills, DIGITS)).trim()
                 : rois.kp    ? (await ocrBuffer(rois.kp,    DIGITS)).trim()
                 : "";
...
async function clipboardSetEmptyADB() { try { await execa(ADB, adbArgs(["shell", "cmd", "clipboard", "set", ""]), { encoding: "utf8" }); } catch {} }
async function clipboardGetADB() { try { const { stdout } = await execa(ADB, adbArgs(["shell", "cmd", "clipboard", "get"]), { encoding: "utf8" }); return (stdout || "").trim(); } catch { return ""; } }
async function waitClipboardNonEmptyADB(maxMs = T_CLIP_ADB, stepMs = T_CLIP_STEP) {
  const end = Date.now() + maxMs;
  while (Date.now() < end) { const t = await clipboardGetADB(); if (t) return t; await sleep(stepMs); }
  return "";
}
...
async function waitClipboardNonEmptyHost(prev = "", maxMs = T_CLIP_HOST, stepMs = T_CLIP_STEP) {
  const end = Date.now() + maxMs;
  while (Date.now() < end) { const t = await clipboardGetHost(); if (t && t !== prev) return t; await sleep(stepMs); }
  return "";
}
async function copyNameIntoTexts(texts) {
  await clipboardSetEmptyADB();
  let hostPrev = "";
  if (USE_HOST_CLIPBOARD) { await clipboardSetEmptyHost(); hostPrev = await clipboardGetHost(); }

  const action = actionCopyFromRegions() || FLOW.copyName;
  const p = regionNameCenter();
  if (action) { logAction("copyName-tap", { action }); await navigateHuman(action); }
  else if (p) { logAction("copyName-tap", { point: p }); await adbTapExact(p.x, p.y, 120); }
  ...
  if (!clip && USE_HOST_CLIPBOARD) clip = await waitClipboardNonEmptyHost(hostPrev, T_CLIP_HOST, T_CLIP_STEP);
  ...
  if (!clip && p) {
    ...
    if (!clip && USE_HOST_CLIPBOARD) clip = await waitClipboardNonEmptyHost(hostPrev, 2500, 200);
  }

  if (clip) { logAction("clipboard-name", { text: clip }); texts.name = clip; }
  else { logAction("clipboard-name", { text: "<empty>" }); }
}
...
async function scanProfileOnce() {
  ...
  // ВАЖЛИВО: в БД летить kills = t4+t5
  const by = parsed.kills || {};
  const killsNum = (Number(by.t4 || 0)) + (Number(by.t5 || 0));
  const cooked = {
    id: parsed.id,
    name: parsed.name,
    power: parsed.power,
  ...
  logAction("scanProfileOnce-result", { parsed: cooked });
  return cooked;
}
...
async function openProfileFromRow(idx, retries = 2) {
  ...
  if (!r) { logAction("rowTap-invalid", { idx }); return false; }
  for (let attempt = 1; attempt <= retries; attempt++) {
    const p = r.rect && hasRect(r.rect) ? pickPointInRect(r.rect) : clampToSafe({ x: r.x, y: r.y });
...
async function openProfileWithFallbacks(baseIdx = BASE_ROW_IDX) {
  ...
  for (const i of candidates) {
    ...
    if (ok) { logAction("fallback-success", { usedIndex: i }); return { opened: true, usedIndex: i }; }
  }
...
async function main() {
  ...
  // Авто-старт KvK, якщо нема активного — щоби відразу були цілі/ваги
  const active = await ensureActiveKvK();
  console.log(`Active KvK: ${active}`);
  ...
  for (let i = 0; i < Math.min(BASE_ROW_IDX, LIST.rows.length) && visited < COUNT; i++) {
    ...
    if (Number.isFinite(pid) && String(pid).length >= 5) {
      ...
      try { const res = await kvkEnsureGoal(pid); if (res) logAction("kvkEnsureGoal", { pid, res }); }
      catch (e) { logAction("kvkEnsureGoal-error", { pid, error: e?.message || String(e) }); }
  ...
  if (ZONE_MODE) {
    try {
      if (ZONE_MODE.mode === "start") {
        await zoneStart(ZONE_MODE.code, { run_id, visited, finished_at: new Date().toISOString() });
        console.log(`Zone ${ZONE_MODE.code}: START stored`);
      } else {
        await zoneFinish(ZONE_MODE.code, { run_id, visited, finished_at: new Date().toISOString() });
        console.log(`Zone ${ZONE_MODE.code}: FINISH stored`);
      }
      const zr = await getZone(ZONE_MODE.code);
...
async function appendBackup(filePath, record) {
  ...
  let arr = [];
  try { arr = JSON.parse(await fs.readFile(filePath, "utf-8")) || []; } catch {}
  arr.push(record);
