...
import {
  ...
  upsertPlayer,
  insertStats,
  kvkEnsureGoal,     // ⬅️ додаємо авто-ensure goal після вставки
  kvkActiveId,       // ⬅️ для логів: чи є активний KvK
  closeDb
} from "./db.pg.js";
...
async function main() {
  ...
  // інформативно: чи є активний KvK
  ...
  while (visited < COUNT) {
    ...
    if (Number.isFinite(pid) && String(pid).length >= 5) {
      ...
      await insertStats(run_id, pid, stats);     // вставка в stats + latest

      // ⬇️ авто-ensure goals (якщо активний KvK існує)
      try {
